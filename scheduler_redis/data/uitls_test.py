    ...
    try:
        ...
        if database_type == 'oracle':
           ...
           segment_type='TABLE' and owner='{schema_name}' and segment_name='{table_name}' """
            ...
            print('table sql query time is :', time.time() - t1)
            for table_name,table_comment in tab_comments_list:
                ...
                t1 = time.time()
                cursor.execute(sql)
                print('table count query time is :',time.time()-t1)
                ...
                if not table_info:
                    ...
                    table_info.table_history_id = table_history_info.id
                    session.flush()
                print('table info add time is :', time.time() - t1)
                t1 = time.time()
                cursor.execute(
                ...
                print('field query time is :', time.time() - t1)
                t1 = time.time()
                merged_info = []
                ...
                print('field format time is :', time.time() - t1)
                ...
                for merged in merged_info:
                    ...
                    session.add(field_history_info)
                    session.flush()
                print('field add time is :', time.time() - t1)
                t1 = time.time()
                field_check_change(columns, table_info.id, data,session)
                print('field check time is :', time.time() - t1)
            t1 = time.time()
            table_check_change([table[0] for table in tab_comments_list], data,session)
            print('table check time is :', time.time() - t1)
        else:
            engine = create_engine(db_url)
        ...
        print('generate_table_template', time.time() - t1)
...
if __name__ == '__main__':
    _data_exploration({'schedule_ids': [425], 'userId': '1', 'deptId': '4', 'datasource_id': 508, 'schedule_id': 425, 'schedule_name': '四川省医院等级评审管理系统（智慧医院）数据库scsyydjpsglxt01_result_table', 'schedule_type': 'immediately'})